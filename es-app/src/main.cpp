	...
	for(int i = 1; i < argc; i++)
	{
		...
		else if (strcmp(argv[i], "--help") == 0 || strcmp(argv[i], "-h") == 0)
		{
			...
			std::cout <<
				...
				"                               0 for unlimited (p)\n"
				"--show-hidden-files            show also hidden files of filessystem, useless\n"
				"                               if --gamelist-only is set (p)\n"
				"--vsync 1|0                    turn vsync on (1) or off (0) (default is on)\n"
				"\nGeneric switches:\n"
				"--help, -h                     summon a sentient, angry tuba\n\n"
				"--home PATH                    directory to use as home path (will contain\n"
				"                               .emulationstation/es_settings.cfg, aso.)\n"
				"\nScrape mode:\n"
				"--scrape                       scrape using command line interface\n\n"
				"Note: Switches marked (p) will be persisted in es_settings.cfg when settings\n"
				"are changed in EmulationStation UI.\n\n"
